...
import { BreakingSystemDetails } from '../models/breaking-system.model';
...
export class myVehicleViewService {
  ...
  getVehicleBreakingSystem(id: number) {
    return this.http.get<{ data: BreakingSystemDetails, status: string }>(`${environment.apiUrl}breakingSystems/${id}`)
      .pipe(
        tap(_ => console.log(`breakingSystems: `, _)),
        catchError(this.handleError<{ data: BreakingSystemDetails, status: string }>('get single breakingSystems'))
      );               
  }
